...
import { z } from "zod";
import { Loader2, Plus, CalendarIcon, Upload, X, Search, Trash2 } from "lucide-react";
...
interface AddComplaintDialogProps { onSuccess: () => void; }
...
const AddComplaintDialog = ({ onSuccess }: AddComplaintDialogProps) => {
  ...
  const filteredDepartments = useMemo(() => {
    ...
    return departments.filter((dept) => dept.name.toLowerCase().includes(departmentSearch.toLowerCase()));
  }, [departments, departmentSearch]);

  const form = useForm<FormData>({
    resolver: zodResolver(formSchema),
    defaultValues: {
      npk: "", reporter_name: "", department: "",
      items: [{ item_name: "", quantity: 1 }],
      description: "", reported_at: new Date(), status: "pending",
    },
  });
  ...
  const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (!file.type.startsWith('image/') || !ALLOWED_IMAGE_TYPES[file.type]) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (photoFile) {
        ...
        const { data: urlData, error: urlError } = await supabase.storage.from("complaint-photos").createSignedUrl(fileName, 3600);
      ...
      const { error } = await supabase.from("complaints").insert({
        ticket_number: ticketData, complaint_code: codeData,
      ...
      toast({ title: "Berhasil", description: "Pengaduan baru berhasil ditambahkan" });
      form.reset();
      setPhotoFile(null);
      ...
      toast({ title: "Gagal", description: error.message || "Gagal menambahkan pengaduan", variant: "destructive" });
    } finally {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="gap-2"><Plus className="h-4 w-4" />Tambah Pengaduan</Button>
      </DialogTrigger>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <FormField control={form.control} name="npk" render={({ field }) => (
              <FormItem><FormLabel>NPK *</FormLabel><FormControl><Input placeholder="Masukkan NPK" {...field} /></FormControl><FormMessage /></FormItem>
            )} />

            <FormField control={form.control} name="reporter_name" render={({ field }) => (
              <FormItem><FormLabel>Nama Pemohon *</FormLabel><FormControl><Input placeholder="Masukkan nama pemohon" {...field} /></FormControl><FormMessage /></FormItem>
            )} />
              ...
              <FormItem>
                ...
                <FormControl>
                  <div className="relative">
                    <div className="flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background cursor-pointer hover:bg-accent/50 transition-colors" onClick={() => setIsDepartmentOpen(!isDepartmentOpen)}>
                      <span className={field.value ? "text-foreground" : "text-muted-foreground"}>{field.value || "Pilih unit kerja"}</span>
                      <Search className="h-4 w-4 opacity-50" />
                    </div>
                    {isDepartmentOpen && (
                      <div className="absolute z-50 w-full mt-1 bg-popover border rounded-md shadow-lg">
                        <div className="p-2 border-b"><Input placeholder="Cari unit kerja..." value={departmentSearch} onChange={(e) => setDepartmentSearch(e.target.value)} className="h-8" autoFocus /></div>
                        <ScrollArea className="h-[200px]">
                          <div className="p-1">
                            {filteredDepartments.length === 0 ? (
                              <div className="py-6 text-center text-sm text-muted-foreground">Unit kerja tidak ditemukan</div>
                            ) : filteredDepartments.map((dept) => (
                              <div key={dept.id} className={`relative flex cursor-pointer select-none items-center rounded-sm px-2 py-2 text-sm outline-none transition-colors hover:bg-accent hover:text-accent-foreground ${field.value === dept.name ? "bg-accent" : ""}`}
                                onClick={() => { field.onChange(dept.name); setIsDepartmentOpen(false); setDepartmentSearch(""); }}>
                                {dept.name}
                              </div>
                            ))}
                          </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <Label className="text-sm font-medium">Daftar Item *</Label>
                {fields.length < 3 && (
                  <Button type="button" variant="outline" size="sm" className="gap-1 h-8 text-xs" onClick={() => append({ item_name: "", quantity: 1 })}>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div className="col-span-2">
                      <FormField control={form.control} name={`items.${index}.item_name`} render={({ field }) => (
                        <FormItem><FormControl><Input placeholder="Nama item" {...field} /></FormControl><FormMessage /></FormItem>
                      )} />
                    </div>
                    <div>
                      <FormField control={form.control} name={`items.${index}.quantity`} render={({ field }) => (
                        <FormItem><FormControl><Input type="number" min={1} placeholder="Jml" {...field} onChange={(e) => field.onChange(parseInt(e.target.value) || 1)} /></FormControl><FormMessage /></FormItem>
                      )} />
                    </div>
                  </div>
                </div>
            ...
            <FormField control={form.control} name="description" render={({ field }) => (
              <FormItem><FormLabel>Keterangan (Opsional)</FormLabel><FormControl><Textarea placeholder="Masukkan keterangan kerusakan" className="resize-none" rows={3} {...field} /></FormControl><FormMessage /></FormItem>
            )} />
            ...
            <div className="space-y-2">
              <Label>Upload Foto Item Rusak (Opsional)</Label>
              {photoPreview ? (
                <div className="relative">
                  <img src={photoPreview} alt="Preview" className="w-full h-48 object-cover rounded-lg border" />
                  <Button type="button" variant="destructive" size="icon" className="absolute top-2 right-2" onClick={removePhoto}><X className="h-4 w-4" /></Button>
                </div>
              ) : (
