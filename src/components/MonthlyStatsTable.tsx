...
const MonthlyStatsTable = ({ data }: Props) => {
  ...
  const availableYears = useMemo(() => {
    ...
    data.forEach((c) => years.add(new Date(c.reported_at).getFullYear()));
  ...
  const monthlyData = useMemo<MonthlyData[]>(() => {
    ...
    // Fill with actual data filtered by year
    ...
    return Array.from(monthlyMap.entries()).map(([monthIndex, stats]) => ({
      month: monthIndex,
            ...
            <Table>
              ...
              <TableBody>
                ...
                <TableRow className="bg-background">
                  ...
                  {monthlyData.map((row) => (
                    <TableCell key={row.month} className="text-center px-2">
                      <span className={`inline-flex items-center justify-center min-w-[2rem] px-2 py-1 rounded-full text-xs font-semibold ${row.pending > 0
                ...
                <TableRow className="bg-muted/20">
                  ...
                  {monthlyData.map((row) => (
                    <TableCell key={row.month} className="text-center px-2">
                      <span className={`inline-flex items-center justify-center min-w-[2rem] px-2 py-1 rounded-full text-xs font-semibold ${row.processing > 0
                ...
                <TableRow className="bg-background">
                  ...
                  {monthlyData.map((row) => (
                    <TableCell key={row.month} className="text-center px-2">
                      <span className={`inline-flex items-center justify-center min-w-[2rem] px-2 py-1 rounded-full text-xs font-semibold ${row.completed > 0
                ...
                <TableRow className="bg-primary/5 border-t-2 border-primary/20">
                  ...
                  {monthlyData.map((row) => (
                    <TableCell key={row.month} className="text-center px-2">
                      <span className={`inline-flex items-center justify-center min-w-[2rem] px-2 py-1 rounded-full text-xs font-bold ${row.total > 0
