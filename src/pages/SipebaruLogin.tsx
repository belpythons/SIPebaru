  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.identifier.trim()) {
      toast({
        ...
        description: "RFID atau Email wajib diisi",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const { data, error } = await supabase.rpc("authenticate_sipebaru_user", {
        login_identifier: formData.identifier.trim(),
      ...
      if (data && data.length > 0) {
        ...
        toast({
          ...
          description: `Selamat datang, ${result.nama}`,
        });

        navigate("/sipebaru/dashboard");
      } else {
      ...
      toast({
        title: "Login Gagal",
        description: error.message || "RFID/Email atau password salah",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
          ...
          <Card className="w-full max-w-md shadow-lg border-0 bg-card/80 backdrop-blur-sm animate-fade-in">
            <CardHeader className="text-center pb-4 sm:pb-6 px-6 sm:px-8 pt-6 sm:pt-8">
              ...
              <p className="text-sm text-muted-foreground mt-2">
                Masuk menggunakan RFID atau Email
              </p>
            </CardHeader>
            <CardContent className="px-6 sm:px-8 pb-6 sm:pb-8">
              <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
                <div className="space-y-2">
                  <Label htmlFor="identifier">RFID atau Email</Label>
                  <Input
                    id="identifier"
                    name="identifier"
                    type="text"
                    placeholder="Masukkan RFID atau Email"
                ...
                <p className="text-center text-sm text-muted-foreground">
                  Belum punya akun?{" "}
                  <Link to="/sipebaru/signup" className="text-primary hover:underline">
                    Daftar di sini
                  </Link>
                </p>
              </form>
